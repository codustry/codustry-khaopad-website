/**
 * ─── codustry.com fork ──────────────────────────────────────
 * SKIPPED: these are structural pins asserting on Khao Pad's STOCK
 * (www) layout and homepage markup — themePrimaryColor wiring, the
 * settings-driven logo, and the m.site_name() hero fallback. This fork
 * replaces both files with the bespoke Codustry design, so the source
 * text they grep for legitimately does not exist here.
 *
 * The underlying engine behaviour is unchanged and still covered
 * upstream; only the markup assertions are fork-specific.
 */
...
describe.skip("(www) layout theme wiring", () => {
...
describe.skip("homepage hero wiring", () => {
...
describe.skip("settings action hex gate", () => {
